...
from .modules.logging import log_tool_call, log_error, log_info, log_warning, log_ws_event, log_exception, log_debug
...
def base64_encode_audio(audio_bytes):
        ...
        audio_frames = []
        # Convert raw bytes to float32 numpy array and add to frames
        audio_chunk = np.frombuffer(audio_bytes, dtype=np.float32)
        audio_frames.append(audio_chunk)

        if not audio_frames:
            log_warning("No audio data available.")
            return None

        # Concatenate all chunks and downsample to 24kHz
        audio_data = np.concatenate(audio_frames)
        audio_data_resampled = audio_data[::2]  # Downsample from 48kHz to 24kHz

        # Convert float32 to PCM16 format
        pcm16_audio = (audio_data_resampled * 32767).astype(np.int16).tobytes()

        # Encode to base64
        pcm_base64 = base64.b64encode(pcm16_audio).decode("utf-8")
        return pcm_base64


# Log runtime info info into the jsonl file
def log_runtime(function_or_name: str, duration: float):
    ...
    log_info(f"⏰ {function_or_name}() took {duration:.4f} seconds")
...
class RealtimeAPI:
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    if self.prompts:
                        ...
                        log_info("Recording started. Listening for speech...")

                    await self.send_audio_loop(websocket)

                    log_info("before await ws_task")

                    # Wait for the WebSocket processing task to complete
                    await ws_task

                    log_info("await ws_task complete")
                ...
                if "keepalive ping timeout" in str(e):
                    ...
                    log_exception("WebSocket connection closed unexpectedly.")
    ...
    async def handle_error(self, event, websocket):
        ...
        #details = event.get
        log_error(f"Error X: {error_message}")
            ...
            while not self.exit_event.is_set():
                ...
                if not self.mic.is_receiving:
                    ...
                    if audio_data and len(audio_data) > 0:
                        ...
                        log_debug("No audio data to send")
                else:
                    await asyncio.sleep(0.1)  # Wait while receiving assistant response
        except KeyboardInterrupt:
            log_info("Keyboard interrupt received. Closing the connection.")
        finally:
            self.exit_event.set()
            self.mic.stop_recording()
            self.mic.close()
